...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let isOpen = false;
  let isCompact = false;
  let expandedHeight = navEl.scrollHeight;

  // 初始
  const getCollapsedHeight = () => (isCompact ? COMPACT_HEIGHT : COLLAPSED_HEIGHT);
  const refreshExpandedHeight = () => {
    expandedHeight = Math.max(navEl.scrollHeight, getCollapsedHeight());
    return expandedHeight;
  };

  // 初始狀態
  gs.set(navEl, { height: getCollapsedHeight(), overflow: "hidden" });
  ...
  function openMenu() {
    ...
    if (reduceMotion) {
      const height = refreshExpandedHeight();
  ...
  function closeMenu() {
    ...
    if (reduceMotion) {
      navEl.classList.remove("open");
      navEl.style.height = `${getCollapsedHeight()}px`;
  ...
  function setCompact(val) {
    ...
    if (!isOpen) gs.set(navEl, { height: getCollapsedHeight() });
